#!/usr/bin/env python3
"""Unit test to check if 1min, 2min, 3min, 4min intervals are available for AMZN"""

from main import LayoffTracker
from datetime import datetime, timezone, timedelta

print("=" * 80)
print("Testing 1min, 2min, 3min, 4min Intervals for Amazon (AMZN)")
print("=" * 80)

tracker = LayoffTracker()

# Test with Dec 4, 2025 11:49 ET = 16:49 UTC (from the image)
article_datetime_utc = datetime(2025, 12, 4, 16, 49, tzinfo=timezone.utc)

print(f"\n📰 Article Details:")
print(f"   Company: Amazon")
print(f"   Ticker: AMZN")
print(f"   Published: {article_datetime_utc.strftime('%a, %b %d, %Y %H:%M')} UTC")
print(f"   Published ET: {(article_datetime_utc - timedelta(hours=5)).strftime('%a, %b %d, %Y %H:%M')} ET")

mock_layoff = {
    'company_name': 'Amazon',
    'stock_ticker': 'AMZN',
    'datetime': article_datetime_utc,
    'date': article_datetime_utc.strftime('%Y-%m-%d'),
    'time': article_datetime_utc.strftime('%H:%M:%S'),
    'url': 'https://test.com/amzn',
    'title': 'AMZN test article'
}

# Check if 1min data is available from Prixe.io
print(f"\n🔍 Step 1: Checking if 1min data is available from Prixe.io...")
article_day = article_datetime_utc.replace(hour=0, minute=0, second=0, microsecond=0)
print(f"   Article day: {article_day.strftime('%Y-%m-%d')}")

# Check days ago
now_utc = datetime.now(timezone.utc)
days_ago = (now_utc - article_day).days
print(f"   Days ago: {days_ago}")

if days_ago <= 30:
    print(f"   ✅ Within 30-day limit for 1min data")
    
    # Try to fetch 1min data
    print(f"   Attempting to fetch 1min intraday data...")
    intraday_1min = tracker._fetch_intraday_data_for_day('AMZN', article_day, interval='1min')
    ...
    if intraday_1min and intraday_1min.get('success'):
        ...
        if intraday_1min:
            error_msg = intraday_1min.get('message', 'Unknown error')
            print(f"      Error: {error_msg}")
        has_1min_data = False
else:
    print(f"   ❌ Outside 30-day limit for 1min data (Prixe.io limit)")
    has_1min_data = False

# Try to fetch 5min data as fallback
print(f"\n🔍 Step 2: Checking 5min data availability (fallback)...")
intraday_5min = tracker._fetch_intraday_data_for_day('AMZN', article_day, interval='5min')
...
try:
    ...
    if has_1min_intervals:
        ...
        print("   → If UI shows N/A, there might be a UI bug or data not being passed correctly")
    else:
        print("❌ 1min, 2min, 3min, 4min intervals are missing data")
        if has_5min_intervals:
            print("   → 5min interval has data, but 1-4min don't")
            if has_1min_data:
                print("   → BUT Prixe.io HAS 1min data - this suggests a code bug!")
            else:
                print("   → Prixe.io doesn't have 1min data for this date")
                print("   → Code is correctly skipping 1-4min when only 5min data is available")
        else:
    ...
    if has_1min_intervals:
        print("✅ 1min, 2min, 3min, 4min intervals ARE available for AMZN")
        print("   The UI should show prices for these intervals")
        print("   If UI shows N/A, check:")
        print("   1. Is the data being passed from backend to frontend?")
        print("   2. Is the UI correctly reading the interval data?")
        print("   3. Are there any JavaScript errors in the browser console?")
    else:
        print("❌ 1min, 2min, 3min, 4min intervals are NOT available for AMZN")
        if has_1min_data:
            print("   ⚠️  WARNING: Prixe.io HAS 1min data, but intervals are missing!")
            print("   This suggests a bug in the extraction logic")
        else:
